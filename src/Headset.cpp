...
Headset::Headset(const Context* context)
    : context(context)
{
    ...
    result = xrEnumerateViewConfigurationViews(xrInstance, xrSystemId, viewType, static_cast<uint32_t>(eyeImageInfos.size()),
    ...
    if (eyeCount == 2u && (eyeImageInfos.at(0).recommendedImageRectWidth != eyeImageInfos.at(1).recommendedImageRectWidth ||
        ...
        util::error(Error::GenericOpenXR, "The two eyes have different resolutions");
        valid = false;
        return;
    }
    ...
    // Create a swapchain and render targets
    {
        ...
        result = xrEnumerateSwapchainImages(swapchain, static_cast<uint32_t>(swapchainImages.size()), &swapchainImageCount, data);
        if (XR_FAILED(result)) {
            util::error(Error::GenericOpenXR);
            valid = false;
            return;
...
Headset::BeginFrameResult Headset::beginFrame(uint32_t& swapchainImageIndex)
{
    ...
    if (sessionState != XR_SESSION_STATE_READY && sessionState != XR_SESSION_STATE_SYNCHRONIZED && sessionState != XR_SESSION_STATE_VISIBLE && sessionState != XR_SESSION_STATE_FOCUSED) {
