    ...
    : context(context)
{
    ...
    descriptorBufferInfos.at(1u).offset = util::align(descriptorBufferInfos.at(0u).range, uniformBufferOffsetAlignment) * static_cast<VkDeviceSize>(modelCount);
    descriptorBufferInfos.at(1u).range = sizeof(StaticVertexUniformData);

    descriptorBufferInfos.at(2u).offset = descriptorBufferInfos.at(1u).offset + util::align(descriptorBufferInfos.at(1u).range, uniformBufferOffsetAlignment);
    ...
    uniformBuffer = new DataBuffer(context, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
