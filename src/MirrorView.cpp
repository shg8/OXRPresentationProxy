...
MirrorView::MirrorView(const Context* context)
    : context(context)
{
...
MirrorView::RenderResult MirrorView::render(uint32_t swapchainImageIndex)
{
    ...
    const VkResult result = vkAcquireNextImageKHR(context->getVkDevice(), swapchain, UINT64_MAX, renderer->getCurrentDrawableSemaphore(),
