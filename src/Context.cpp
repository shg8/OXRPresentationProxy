...
Context::Context()
{
    ...
    // Create an OpenXR debug utils messenger for validation
    {
        ...
        constexpr XrDebugUtilsMessageTypeFlagsEXT typeFlags = XR_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_TYPE_CONFORMANCE_BIT_EXT;

        constexpr XrDebugUtilsMessageSeverityFlagsEXT severityFlags = XR_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | XR_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
        ...
        const XrResult result = xrCreateDebugUtilsMessengerEXT(xrInstance, &debugUtilsMessengerCreateInfo, &xrDebugUtilsMessenger);
        if (XR_FAILED(result)) {
            util::error(Error::GenericOpenXR);
            valid = false;
            return;
    ...
    // Get the required Vulkan instance extensions from OpenXR and add them
    {
        ...
        result = xrGetVulkanInstanceExtensionsKHR(xrInstance, systemId, count[0], &count[0], buffer.data());
        if (XR_FAILED(result)) {
            util::error(Error::GenericOpenXR);
            valid = false;
            return;
    ...
    // Create a Vulkan debug utils messenger for validation
    {
        ...
        if (vkCreateDebugUtilsMessengerEXT(vkInstance, &debugUtilsMessengerCreateInfo, nullptr, &vkDebugUtilsMessenger) != VK_SUCCESS) {
            util::error(Error::GenericVulkan);
            valid = false;
            return;
        }
...
bool Context::createDevice(VkSurfaceKHR mirrorSurface)
{
    ...
    std::vector<VkExtensionProperties> supportedVulkanDeviceExtensions;
    {
        ...
        if (vkEnumerateDeviceExtensionProperties(physicalDevice, nullptr, &deviceExtensionCount,
                supportedVulkanDeviceExtensions.data())
            != VK_SUCCESS) {
            util::error(Error::GenericVulkan);
            return false;
        }
    ...
    std::vector<const char*> vulkanDeviceExtensions;
    {
        ...
        std::string buffer;
        buffer.resize(count[0]);
        result = xrGetVulkanDeviceExtensionsKHR(xrInstance, systemId, count[0], &count[0], buffer.data());
        if (XR_FAILED(result)) {
            util::error(Error::GenericOpenXR);
            return false;
        }
